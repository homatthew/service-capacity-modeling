...
def _ebs_page_cache_overhead(
    ...
    # the desires to compute the latency ratio. Note: avg_read_latency_ms
    # comes from the user's SLO estimate (desires), not measured actuals.
...
def _estimate_cassandra_cluster_zonal(  # pylint: disable=too-many-positional-arguments
    ...
    if experimental_memory_model and is_ebs and working_set_mem_gib > 0:
        essential_memory_gib = 0  # page cache is soft for EBS
    else:
